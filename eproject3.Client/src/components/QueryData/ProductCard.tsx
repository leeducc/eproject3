...
export default function ProductCardGrid() {
    const { client } = useClient() as unknown as { client: JsonServiceClient }
    ...
    const currentType = routeToType[location.pathname as keyof typeof routeToType]
    const currentCategoryId = routeToCategoryId[location.pathname as keyof typeof routeToCategoryId]
...
return (
        <div className="flex">
            <aside className="w-72 p-4 space-y-4 bg-white shadow-md">
                <input
                    type="text"
                    placeholder="Search..."
                ...
                <div>
                    ...
                    <input type="number" placeholder="Min" className="w-full mb-2 p-1 border rounded"
                ...
                <div>
                    ...
                    <select className="w-full p-2 border rounded" value={creatorId ?? ''} onChange={e => setCreatorId(Number(e.target.value) || null)}>
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {allGenres.map(g => (
                            <button
                                key={g.id}
                                className={`px-3 py-1 text-sm border rounded-full ${selectedGenres.includes(g.id) ? 'bg-blue-600 text-white' : 'bg-gray-100'}`}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 p-4 flex-1">
                {products.map(product => (
                    <a
                        ...
                        className="relative group w-full max-w-xs mx-auto overflow-hidden rounded-lg bg-white shadow-md"
                    >
                        <div className="relative h-60 overflow-hidden">
                            <img
                                className="object-cover w-full h-full transition duration-300 ease-in-out group-hover:blur-sm"
                                src={product.image}
                                alt={product.title}
                            />
